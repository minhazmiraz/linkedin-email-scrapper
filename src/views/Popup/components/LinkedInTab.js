...
const LinkedInTab = (props) => {
			...
			{Object.keys(unsavedUsersProfile).length > 0 &&
				(isUpdating ? (
					...
					<Tooltip title="Fetch email" placement="left">
						...
						>
							{getBadgeWithValue(unsavedUsersProfile, "primary", <UpdateOutlined />)}
						</Fab>
					</Tooltip>
				))}
		</div>
	...
	const noUsersProfile = (
		...
		>
			<i>Goto linkedin connection or search page and click the refresh button</i>
		</p>
	);
	...
	return (
		<div className="linkedin-tab">
			{isLinkedInTab && Object.keys(usersProfile).length > 0 ? (
				<div>
					<Alert severity="info">{Object.keys(usersProfile).length} profile found</Alert>
