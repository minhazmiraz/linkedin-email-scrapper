...
function Home() {
	...
	const filterUsersProfile = () => {
		...
		let input =
			Object.keys(savedUsersProfile).length + Object.keys(unsavedUsersProfile).length ===
		...
		let [saved, unsaved] = Object.entries(input).reduce(
			([saved, unsaved], user) => {
				...
				if (profile?.email?.length >= 0) return [{ ...saved, [user[0]]: profile }, unsaved];
	...
	const handleOnClickUpdate = (data) => {
		sentMessageToBackground({ to: "background", query: "UPDATE_EMAIL", data }, (res) =>
			console.log(res)
		);
	};
